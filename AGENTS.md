...
- Checkout must fail closed to an honest boundary dialog; do not collect submitted personal/payment data and do not link to a fake checkout.
...
- Product imagery in `src/assets/images/` is original AI-generated concept photography; disclose that in the preview notes/README, not as authentic product photography.
...
- Checkout boundary: deterministic dialog explaining that ordering is not enabled and listing the exact information needed to activate it. No application-level submitted-data collection; hosting request logs are disclosed.
...
- No cookie banner or embedded analytics while no application/ad tracking exists; do not imply that the hosting provider has no request logs.
...
- CI on pushes/PRs and a same-SHA, verification-gated GitHub Pages deployment. `public/CNAME` is only an artifact declaration. Verify the parent domain in the GitHub account, claim the hostname on the repository, and only then publish its DNS CNAME. Pages should remain usable at its GitHub project URL until that activation gate closes.
